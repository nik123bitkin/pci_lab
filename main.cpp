...
{
    for(auto item : PciVenTable){
        if (vID == item.VendorId) {
...
{
    for(auto item : PciDevTable){
        if (vID == item.VendorId && dID == item.DeviceId) {
...
{
    for(auto item : PciSubTable){
        if (venID == item.VendorId && devID == item.DeviceId
            && subVenID == item.SubVendorId && subDevID == item.SubDeviceId) {
            return item.SubDeviceName;
        }
    }
    return nullptr;
}
...
{
    for(auto item : PciClassCodeTable){
        if (bClass == item.BaseClass) {
            return item.BaseDesc;
        }
    }
    return nullptr;
}
...
{
    for(auto item : PciClassCodeTable){
        if (bClass == item.BaseClass && sClass == item.SubClass) {
            return item.SubDesc;
        }
    }
    return nullptr;
...
{
    for(auto item : PciClassCodeTable){
        if (bClass == item.BaseClass && sClass == item.SubClass && srlPI == item.ProgIf) {
            return item.ProgDesc;
        }
    }
    return nullptr;
}